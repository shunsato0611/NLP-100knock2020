...
class BiRNN(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, vocab_size, tagset_size,pre_trained_embedding):
        ...
        self.birnn = nn.RNN(embedding_dim, hidden_dim, num_layers=2, bidirectional=True,)
    ...
    def forward(self,sentence,word_seq_lengths):
        ...
        #outputはconcatされてるもの
        forward_h = h_n[-2]#前向きの最後
        #0でもうまく行ったのは一層だったから
        #-1,-2にしないと多層化の時にバグになる
        back_h = h_n[-1]#後ろ向きの最後
        birnn_out = torch.cat((forward_h,back_h),dim=1)#catしてhidden_dim*2次元
        #引数はちゃんと書こう
        #h_nは各時刻の情報がpackされている情報がくる
        #そのまま使うとpackの最後の情報がピンポイントで使えて便利
        #多層化とはRNNのことなので直すように
        tag_space = self.hidden2tag(birnn_out)
